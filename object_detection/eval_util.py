...
def visualize_detection_results(crop_num,
                                result_dict,
                                tag,
                                global_step,
                                categories,
  ...
  #CHEN: test for hotspot eval
  groundtruth_boxes = result_dict[input_fields.groundtruth_boxes]
  #CHEN: visualize for hotspot region (1/3 of the middle part)
  gt_hotspot_region = np.copy(groundtruth_boxes)
  eval_hotspot_region = np.copy(detection_boxes)

  cleaned_groundtruth_boxes = np.copy(groundtruth_boxes)
  cleaned_detection_boxes = np.copy(detection_boxes)
  
  offset = 23
  #gt hotspot region
  for i in range(np.shape(gt_hotspot_region)[0]):
    middle_x = groundtruth_boxes[i,0] + (groundtruth_boxes[i,2] - groundtruth_boxes[i,0])/2
    middle_y = groundtruth_boxes[i,1] + (groundtruth_boxes[i,3] - groundtruth_boxes[i,1])/2
    
    if middle_x < offset or middle_x > 256 - offset \
    or middle_y < offset or middle_y > 256 - offset:
      #print("REDUNDANT")
      cleaned_groundtruth_boxes = np.delete(groundtruth_boxes,i,0) #remove the redundant box


    x_offset = 11#(groundtruth_boxes[i,2] - groundtruth_boxes[i,0])/6
    y_offset = 11#(groundtruth_boxes[i,3] - groundtruth_boxes[i,1])/6
    
    if middle_x >= offset or middle_x <= 256 - offset \
    or middle_y >= offset or middle_y <= 256 - offset:
      gt_hotspot_region[i,0] = middle_x - x_offset
      gt_hotspot_region[i,1] = middle_y - y_offset
      gt_hotspot_region[i,2] = middle_x + x_offset
      gt_hotspot_region[i,3] = middle_y + y_offset

  for i in range(np.shape(eval_hotspot_region)[0]):
    middle_x = detection_boxes[i,0] + (detection_boxes[i,2] - detection_boxes[i,0])/2
    middle_y = detection_boxes[i,1] + (detection_boxes[i,3] - detection_boxes[i,1])/2

    if middle_x < offset or middle_x > 256 - offset \
    or middle_y < offset or middle_y > 256 - offset:
      #print("REDUNDANT")
      cleaned_detection_boxes = np.delete(detection_boxes,i,0) #remove the redundant box


    x_offset = 11#(detection_boxes[i,2] - detection_boxes[i,0])/6
    y_offset = 11#(detection_boxes[i,3] - detection_boxes[i,1])/6
    
    if middle_x >= offset or middle_x <= 256 - offset \
    or middle_y >= offset or middle_y <= 256 - offset:
      eval_hotspot_region[i,0] = middle_x - x_offset
      eval_hotspot_region[i,1] = middle_y - y_offset
      eval_hotspot_region[i,2] = middle_x + x_offset
      eval_hotspot_region[i,3] = middle_y + y_offset
  ...
  #CHEN: eval the hotspot, (x-x') && (y-y') < 1/3*length
  vis_utils.eval_hotspot(detection_classes,
                        detection_scores,
                        category_index,
                        cleaned_groundtruth_boxes,
                        cleaned_detection_boxes,
                        min_score_thresh=min_score_thresh,
                        num_examples=num_examples
                        )

  '''for one stage testing'''
  '''
  hotspot,false_alarm,miss_detected = vis_utils.one_stage_eval_hotspot(detection_classes,
                                                            detection_scores,
                                                            category_index,
                                                            groundtruth_boxes,
                                                            detection_boxes,
  ...
  '''
  accuracy,sum_false_alarm,sum_detection_area,sum_FA_detection_area,sum_gt_area = vis_utils.eval_area_hotspot(detection_classes,
                                                    detection_scores,
                                                    category_index,
                                                    groundtruth_boxes,
                                                    detection_boxes,
  ...
  print("Area accuracy:",float(sum(sum_detection_area))/float(sum(sum_gt_area)))
  '''
  '''
  box_saver,num_counter = vis_utils.eval_area_hotspot_v2(detection_classes,
                                              detection_scores,
                                              category_index,
                                              groundtruth_boxes,
                                              detection_boxes,
  ...
  #print("box_saver'shape:",np.shape(box_saver))
  #print("box_saver:",box_saver)
  print("num_counter & crop_num:",num_counter,",",crop_num)
  if num_counter == crop_num:
    np.savetxt("/research/byu2/rchen/proj/gds/large_scale_detected_label_3.csv",box_saver,delimiter=",")
  '''
  if export_dir:
    ...
    try:
      for batch in range(int(num_batches)):
        ...
        if not batch_processor:
          ...
          #print("298:groundtruth_boxes:",len(result_dict['groundtruth_boxes']))
          #print("299:detection_boxes:",len(result_dict['detection_boxes']))
