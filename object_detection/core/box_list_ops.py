...
'''CHEN: detection on middle part of hotspot mask'''
def scale_to_middle(boxlist, scope=None):
  with tf.name_scope(scope,'Scale_to_middle'):
    y_min, x_min, y_max, x_max = tf.split(
        value=boxlist.get(), num_or_size_splits=4, axis=1)
    delta_x = (x_max - x_min) / 3
    delta_y = (y_max - y_min) / 3
    ...
    middle_boxlost = box_list.BoxList(
        tf.concat([y_min, x_min, y_max, x_max], 1))
    return _copy_extra_fields(middle_boxlost, boxlist)
