...
class WeightedSmoothL1LocalizationPlusLoss(Loss):
  """Smooth L1 localization loss function aka Huber Loss..
  ...
  def _compute_loss(self, prediction_tensor, target_tensor, decoded_prediction_tensor, decoded_target_tensor, weights):
    """Compute loss function.
    ...
    predicted_boxes = box_list.BoxList(tf.reshape(decoded_prediction_tensor, [-1, 4]))
    target_boxes = box_list.BoxList(tf.reshape(decoded_target_tensor, [-1, 4]))
    ...
    if tf.to_int32(tf.is_nan(IoUloss)) == 1 or tf.to_int32(tf.is_inf(IoUloss)) == 1:
      return SmoothL1loss
    else:
