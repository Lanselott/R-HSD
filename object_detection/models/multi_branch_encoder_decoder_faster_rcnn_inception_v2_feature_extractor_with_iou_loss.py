...
class FasterRCNNInceptionV2FeatureExtractor(
  ...
  def _extract_proposal_features(self, preprocessed_inputs, scope):
    ...
    Block 1
    '''
    encoder_1_conv = slim.conv2d(
                  preprocessed_inputs, depth(32), [3, 3],
                  weights_initializer=trunc_normal(0.09),
                  scope='encoder_c1_conv')
    encoder_2_conv = slim.conv2d(
                  encoder_1_conv, depth(64), [5, 5],
                  weights_initializer=trunc_normal(0.09),
                  scope='encoder_c2_conv')
    encoder_3_conv = slim.conv2d(
                  encoder_2_conv, depth(128), [5, 5],
                  weights_initializer=trunc_normal(0.09),
    ...
    encoder_1_conv_dilated_2 = slim.conv2d(
                  preprocessed_inputs, depth(32), [3, 3], rate = 5,
                  weights_initializer=trunc_normal(0.09),
                  scope='encoder_1_conv_dilated_2')
    encoder_2_conv_dilated_2 = slim.conv2d(
                  encoder_1_conv_dilated_2, depth(64), [5, 5],  rate = 5,
                  weights_initializer=trunc_normal(0.09),
                  scope='encoder_2_conv_dilated_2')
    encoder_3_conv_dilated_2 = slim.conv2d(
                  encoder_2_conv_dilated_2, depth(128), [5, 5],  rate = 5,
                  weights_initializer=trunc_normal(0.09),
                  scope='encoder_3_conv_dilated_2')
    
    encoder_concat = tf.concat([encoder_3_conv,encoder_3_conv_dilated_1,encoder_3_conv_dilated_2],axis=3)
    
    #decoder
    decoder_3_deconv = slim.conv2d_transpose(
                    encoder_concat, depth(64), [5,5],
                    weights_initializer=trunc_normal(0.09),
