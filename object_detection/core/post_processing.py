...
def soft_clip_non_max_suppression(boxes, scores, max_output_size, iou_threshold, soft_nms_sigma):
  method = 2
  sigma = soft_nms_sigma
  Nt = iou_threshold
  score_threshold = float('-inf')
  selected_indices = tf.convert_to_tensor(tf.zeros([max_output_size], tf.int32), dtype=tf.int32)
  logging.info("check")
  if boxes.get_shape().as_list()[0] == None:
    pass
    # FIXME: Currently hard coded, it's hard to modify due to the shit tf1.14
    # selected_indices = tf.placeholder(dtype=tf.int32, shape=(None,))
  else:
    pos = 0
    maxscore = 0
    maxpos = 0
	...
	# NMS iterations, note that N changes if detection boxes fall below threshold
      while pos < N:
        ...
        if iw > 0:
		    ...
		    # update N
                if boxes[pos, 4] < score_threshold:
        ...
        pos = pos + 1
    # selected_indices = tf.convert_to_tensor([i for i in range(N)])
    selected_indices = tf.convert_to_tensor(tf.ones([max_output_size], tf.int32), dtype=tf.int32).eval()
    # print_output = tf.Print("tensors",selected_indices)
...
def multiclass_non_max_suppression(boxes,
  ...
  with tf.name_scope(scope, 'MultiClassNonMaxSuppression'):
    ...
    for class_idx, boxes_idx in zip(range(num_classes), boxes_ids):
      ...
      '''CHEN: test for middle hotspot area'''
      apply_hotspot_nms = 1

      middle_box = box_list_ops.scale_to_middle(boxlist_filtered)
      '''TODO:Add clip soft non maximum supression'''
      apply_soft_hotspot_nms = 0

      assert apply_hotspot_nms != apply_soft_hotspot_nms

      if apply_hotspot_nms == 1:
        selected_indices = tf.image.non_max_suppression(
            middle_box.get(),
            middle_box.get_field(fields.BoxListFields.scores),
            max_selection_size,
            iou_threshold=iou_thresh)
        if selected_indices.get_shape().as_list()[0]!= None:
          selected_indices = tf.Print("tensors",selected_indices)
      elif apply_soft_hotspot_nms == 1:
        selected_indices = soft_clip_non_max_suppression(
            middle_box.get(),
            middle_box.get_field(fields.BoxListFields.scores),
            max_selection_size,
            iou_threshold=iou_thresh,
